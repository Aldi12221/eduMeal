    ...
    artikelData.forEach(a => {
      artikelContainer.innerHTML += `
        <article class="border rounded-lg p-4 shadow hover:shadow-lg transition bg-white">
          ...
          <h3 class="font-bold text-lg mb-2 text-green-700 dark:text-green-400">${a.judul}</h3>
          <p class="text-gray-600 text-sm dark:text-gray-400">${a.deskripsi}</p>
          <button class="mt-3 bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 transition text-sm">
            Baca Selengkapnya
          </button>
        </article>
      `;
