  ...
  cekBtn.addEventListener("click", () => {
    ...
    detailGizi.innerHTML = `
      ...
      ${tidakDitemukan.length > 0 
        ? `<p class="text-red-600 text-sm mt-2">⚠️ Data tidak ditemukan untuk: ${tidakDitemukan.join(", ")}</p>` 
        : ""}
    `;

    // Analisis keseimbangan sederhana
    let pesan = "";
    if (total.protein < 15) pesan = "Kandungan protein rendah, tambahkan lauk seperti ayam atau telur 🥚";
    else if (total.lemak > 30) pesan = "Lemak cukup tinggi, kurangi gorengan atau santan 🥘";
    else if (total.karbo > 100) pesan = "Karbohidrat tinggi, coba kurangi nasi atau mie 🍚";
